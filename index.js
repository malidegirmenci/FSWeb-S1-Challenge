...
function fenomenGonderimSayisi(arr,nameFenomen){
  for(let i = 0; i < arr.length;i++){
    if(arr[i].profile === nameFenomen){
      return arr[i].posts
    }
  }
}
...
function platformaGoreCokGonderiYapanFenomen(arr,platform){
  const samePlatformArr = [];
  for(let i = 0; i < arr.length; i++){
    if(arr[i].platform === platform){
      if(arr[i].posts !== "NA"){
        samePlatformArr.push(arr[i])
      }
    }
  }
  let max = 0;
  let hasMaxPostProfile = "";
  for(let i = 0; i < samePlatformArr.length;i++){
      if(max < samePlatformArr[i].posts){
        max = samePlatformArr[i].posts
        hasMaxPostProfile = samePlatformArr[i].profile
      }
  }
  return hasMaxPostProfile
}
